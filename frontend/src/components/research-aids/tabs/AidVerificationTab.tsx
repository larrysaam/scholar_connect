...
import { useResearchAidProfile } from "@/hooks/useResearchAidProfile"; // Changed hook

interface AidVerificationTabProps {
  researchAidId: string; // Changed prop name
}

const AidVerificationTab = ({ researchAidId }: AidVerificationTabProps) => { // Changed component and prop name
  const { t } = useLanguage();
  const { researchAid, loading, error } = useResearchAidProfile(researchAidId); // Changed variable name and hook
  ...
  if (!researchAid || !researchAid.verifications) { // Changed variable name
    return <div>No verification data available.</div>;
  }

  const verifications = researchAid.verifications; // Changed variable name
  ...
  const verificationItems = [
    {
      ...
      status: getDisplayStatus(verifications.academic, verifications.education),
      icon: <Award className="h-5 w-5" />,
      documents: verifications.education || [],
    ...
    },
    {
      ...
      status: getDisplayStatus(verifications.publication, verifications.publications),
      icon: <FileText className="h-5 w-5" />,
      documents: verifications.publications || [],
    ...
    },
    {
      ...
      status: getDisplayStatus(verifications.institutional, verifications.employment),
      icon: <Building className="h-5 w-5" />,
      documents: verifications.employment || [],
  ...
  const getOverallStatus = () => {
    const academicStatus = getDisplayStatus(verifications.academic, verifications.education);
    const publicationStatus = getDisplayStatus(verifications.publication, verifications.publications);
    const institutionalStatus = getDisplayStatus(verifications.institutional, verifications.employment);
  ...
  const verifiedCount = [
    getDisplayStatus(verifications.academic, verifications.education),
    getDisplayStatus(verifications.publication, verifications.publications),
    getDisplayStatus(verifications.institutional, verifications.employment)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          This research aid's credentials are verified through our comprehensive three-tier validation process.
        </p>
      </div>
      ...
      <Card className="bg-blue-50 border-blue-200">
        ...
        <CardContent>
          <div className="text-sm text-blue-700">
            ...
            <p className="mb-2">
              This research aid has been verified through our rigorous process that includes:
            </p>
...
export default AidVerificationTab;
